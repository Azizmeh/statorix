...
SELECT 
  e.siret,
  e."dateCreationEtablissement" as "dateCration",
  ...
  case 
  when e."codeCommuneEtablissement" is null then concat('Commune génerique ', p.nom)
  else c.nom 
  end as commune,
  case 
  when e."codeCommuneEtablissement" is null then concat('Département génerique ', p.nom)
  else d.nom 
  end as departement,
  case 
  when e."codeCommuneEtablissement" is null then concat('Région génerique ', p.nom)
  else r.nom 
  end as region,
  case 
...
left join categorie_juridique cj2 on cj2.code=cj1.code_parent
left join categorie_entreprise ce on ce.code=ul."categorieEntreprise"
left join pays p on p.nom = e."codePaysEtrangerEtablissement"
